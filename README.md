...
```java
// KIS API, LLM API, 분석 오케스트레이션을 독립 스레드 풀로 분리
...
```java
// LLM API 무응답 시 스레드 자원 보호
List<LLMAnalysisResult> results = futures.stream()
    .map(f -> f.orTimeout(60, TimeUnit.SECONDS))  // 60초 초과 시 해당 종목만 제외
...
- `@Async`로 분리된 분석 작업이 완료되기 전 Kafka ACK가 발생하여 서버 재시작 시 처리 중인 메시지 유실

### 해결 방안: Kafka Message Queue + ACK 시점 제어
...
Consumer는 분석 로직을 동기적으로 실행하고, 완료 후 ACK를 커밋합니다.
서버 재시작 시 ACK되지 않은 메시지는 Kafka가 자동 재배달합니다.
...
analysis-requests 토픽
        ...
        │
        └─ Consumer 처리 실패 (JSON 파싱 오류 / LLM 실패 / KIS 실패 등)
                    │
...
public void retryFailedRequests() {
    ...
    for (FailedAnalysisRequest request : failedRequests) {
        if (request.getRetryCount() < MAX_RETRY_COUNT) {
            kafkaTemplate.send(ANALYSIS_TOPIC, analysisId, message).get();  // 전송 완료 확인
            request.incrementRetryCount();
            request.markAsProcessed();  // 전송 성공 후 완료 처리
        }
    }
}
```
