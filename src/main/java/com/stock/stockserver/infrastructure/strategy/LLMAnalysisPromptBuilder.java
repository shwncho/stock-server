...
public class LLMAnalysisPromptBuilder {
    ...
    public String build(StockDataDto stockData) {
        ...
        String currencyCode = isOverseas ? "USD" : "KRW";
        String priceFormat = isOverseas ? "%,.2f" : "%,.0f";

        String template = """
                다음은 %s의 거래량 상위 종목 중 하나의 데이터입니다.
                통화 단위는 모두 %s(%s)이며, 분석 답변에서도 반드시 같은 통화로 표기하세요.
