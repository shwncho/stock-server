...
public class KafkaAnalysisRequestPublisher implements AnalysisRequestPublisher {
    ...
    public void publish(String analysisId) {
        String message = toMessage(analysisId);
        kafkaTemplate.send(ANALYSIS_TOPIC, analysisId, message);
        log.info("Analysis request sent to Kafka: analysisId={}", analysisId);
    }

    @Override
    public void publishAndWait(String analysisId) throws Exception {
        String message = toMessage(analysisId);
        kafkaTemplate.send(ANALYSIS_TOPIC, analysisId, message).get();
        log.info("Analysis request re-sent to Kafka: analysisId={}", analysisId);
    }

    private String toMessage(String analysisId) {
        try {
            return objectMapper.writeValueAsString(AnalysisEvent.of(analysisId));
